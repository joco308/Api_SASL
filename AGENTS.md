...
- ASP.NET Core **Minimal APIs** (no Controllers). Modules wire at `Program.cs:164-171`.
...
## Architecture
- **Minimal APIs** — every module registers via `Map*Endpoints(this IEndpointRouteBuilder)` at `Program.cs:164-171`.
...
**Empty scaffolds** (2): Administradores, Cobros — directory skeleton, no `.cs` files.

Note: endpoint file naming is inconsistent (`UsuariosEndpoints.cs`, `EndpointClientes.cs`, `EnpointTrabajadores.cs`).  
Module directory is `Provedores` (with 'd') but endpoint registration is `MapProveedoresEndpoints` (with 'dd') at `Program.cs:168`.
...
- Policies: `PersonalAutorizado` (Gerente/Administrador, `Program.cs:128-129`), `Cliente` (`Program.cs:130-131`), `Trabajador` (`Program.cs:132-133`).
- `JwtSecurityTokenHandler.DefaultInboundClaimTypeMap.Clear()` at `Program.cs:43`.
...
## Notable quirks
- CORS: only `http://localhost:3000` (policy `PermitirNEXTJS`, credentials allowed, `Program.cs:55-61`).
- Config sections: `SmtpSettings` (`appsettings.json:12-17`), `Jwt` (Key/Issuer/Audience, `appsettings.json:18-22`).
- `GET /api/diagnostico-final` exists only under `#if DEBUG` (`Program.cs:234-246`) — dumps `ClaimsPrincipal` for auth debugging.
- `GET /Api/Catalogos/{nombre}` is inline at `Program.cs:175-185` (not a module), auth-required, routes through `IUsuariosLogica.ObtenerCatalogoPorDominioAsync`.
